...
for j = 1:8
    ...
    yhat = at+bt+ct;
    err(1,j) = sqrt(mean((yt-yhat').^2)); %求方差
    alpha = alpha+0.1;
end
minS=err(1,8);min = 8;
for k = 1:8
...
st3(1) = alpha*st2(1)+(1-alpha)*st0;
 ...
 ct = 0.5*alpha^2/(1-alpha)^2*(st1-2*st2+st3);
 yhat = at+bt+ct;
 xlswrite('CPI预测.xls',[st1',st2',st3'],'Sheet1','B2') %把数据写在前三列
 xlswrite('CPI预测.xls',yhat','Sheet1','E3');%把数据写在第4列第2行开始的位置
